...
function PLUGIN:Init()
	...
	command.AddChatCommand( "lanterns_on", self.Object, "cmdLanternsOn" )
	command.AddChatCommand( "lanterns_off", self.Object, "cmdLanternsOff" )
end
...
local function start_sphereCastLights(triggerbase,pos,radius)
	...
	while (it:MoveNext()) do
		if(it.Current:GetComponentInParent(global.WorldItem._type)) then
			if(it.Current:GetComponentInParent(global.WorldItem._type).item) then
				if(tostring(it.Current:GetComponentInParent(global.WorldItem._type).item.info.shortname) == "lantern") then
					it.Current:GetComponentInParent(global.WorldItem._type).item:SwitchOnOff(true,nil)
					add_ZoneLight(triggerbase,it.Current:GetComponentInParent(global.WorldItem._type).item)
				end
			end
		end
	end
...
local function stop_sphereCastLights(triggerbase,pos,radius)
	...
	while (it:MoveNext()) do
		if(it.Current:GetComponentInParent(global.WorldItem._type)) then
			if(it.Current:GetComponentInParent(global.WorldItem._type).item) then
				if(tostring(it.Current:GetComponentInParent(global.WorldItem._type).item.info.shortname) == "lantern") then
					if(canSwitchOff(it.Current:GetComponentInParent(global.WorldItem._type).item)) then
						it.Current:GetComponentInParent(global.WorldItem._type).item:SwitchOnOff(false,nil)
					end
				end
			end
		end
...
local function switchAllLanterns(light)
	allWorldItems = UnityEngine.Object.FindObjectsOfTypeAll(global.WorldItem._type)
	for i=0, allWorldItems.Length-1 do
		if(allWorldItems[i].item and tostring(allWorldItems[i].item.info.shortname) == "lantern") then
			allWorldItems[i].item:SwitchOnOff(light,nil)
		end
	end
end
